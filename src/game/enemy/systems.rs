...
pub fn spawn_enemies(
    ...
    for _ in 0..NUMBER_OF_ENEMIES {
        ...
        let random_direction_x = match random::<bool>() {
            true => random::<f32>(),
            false => -random::<f32>(),
        };

        let random_direction_y = match random::<bool>() {
            true => random::<f32>(),
            false => -random::<f32>(),
        };

        let direction = Vec2::new(random_direction_x, random_direction_y).normalize();
...
pub fn update_enemy_direction(
    ...
    for (transform, mut enemy) in enemy_query.iter_mut() {
        ...
        if direction_changed {
            let sound_effect = match random::<bool>() {
